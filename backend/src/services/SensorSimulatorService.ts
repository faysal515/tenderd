...
export class SensorSimulatorService {
  private vehicles = [
    {
      ecuDeviceId: "device-001", // Hardcoded for ECU device
      latitude: 37.7749,
      longitude: -122.4194,
  ...
  private generateSensorData(vehicle: any) {
    ...
    vehicle.odometerReading += Math.random() * 10;
    vehicle.engineHours += 0.1;
    vehicle.fuelLevel = Math.random() > 0.5 ? "Full" : "Half";
